...
impl App {
    pub(crate) fn preview_toolbar(&mut self, ui: &mut egui::Ui) {
        ...
        ui.add_enabled_ui(has_preview, |ui| {
            ...
            if ui
                .add_sized(btn_size, crate::components::button::icon("−"))
                .clicked()
            {
            ...
            ui.add_enabled_ui(has_next, |ui| {
                if ui
                    .add_sized(btn_size, crate::components::button::icon("\u{25B6}"))
                    .clicked()
                {
                    new_page = page + 1;
                }
            ...
            ui.add_enabled_ui(has_prev, |ui| {
                if ui
                    .add_sized(btn_size, crate::components::button::icon("\u{25C0}"))
                    .clicked()
                {
            ...
            if ui
                .add_sized(btn_size, crate::components::button::icon("\u{25B2}"))
                .on_hover_text("Up")
                .clicked()
            {
                let offset = self.active_tab().scroll_offset;
            ...
            if ui
                .add_sized(btn_size, crate::components::button::icon("\u{25BC}"))
                .on_hover_text("Down")
                .clicked()
            {
                let offset = self.active_tab().scroll_offset;
            ...
            if new_page != page {
                ...
                if let Some(size) = self.active_tab().preview.image_size {
                    ...
                    self.active_tab_mut().scroll_request =
                        Some(egui::vec2(0.0, new_page as f32 * (display_size.y + 16.0)));
                }
            }
            ...
            if ui
                .add_sized(
                    btn_size,
                    crate::components::button::icon("↖").selected(!pan_mode),
                )
                .clicked()
            {
                pan_mode = false;
            ...
            if let Some(size) = image_size {
                ...
                let output = scroll_area.show(ui, |ui| {
                    ...
                    for p in 0..num_pages {
                        ...
                        if bottom_y < viewport_top - page_height
                            || top_y > viewport_bottom + page_height
                        {
                            // Add placeholder space for off-screen pages
                            ui.allocate_exact_size(
                                egui::vec2(display_size.x, display_size.y + 16.0),
                                egui::Sense::hover(),
                            );
                            continue;
                        }
                        ...
                        ui.horizontal(|ui| {
                            ...
                            frame.show(ui, |ui| {
                                if let Some(tex) = self.active_tab().preview_textures.get(&p) {
                                    let image = Image::from_texture(egui::load::SizedTexture::new(
                                        tex.id(),
                                        display_size,
                                    ));
                                    ui.add(image);
