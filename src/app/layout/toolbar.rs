...
impl App {
    pub(crate) fn toolbar(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            let compile_enabled =
                !self.tabs.is_empty() && self.active_tab().buffer.path().is_some();
            ...
            let compile_btn = crate::components::button::toolbar(
                button_text,
                match resolved {
            ...
            let llm_btn = crate::components::button::toolbar(
                llm_btn_text,
                match resolved {
            ...
            let llm_enabled = !self.tabs.is_empty()
                && !self.llm_correction_in_progress
                && self.llm_configured
                && self.active_tab().error_message.is_some();
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                let mut btn = egui::Button::new(icon_text)
                ...
                let btn = if preview_active {
                    btn.selected(true)
                } else {
                    btn
                };
                let resp = ui
                    .add_enabled(has_tabs, btn)
                    .on_hover_text("Toggle Preview");
                if resp.clicked() {
